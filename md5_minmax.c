...
#define num_hashes 62*62*31
#define len_postfix 7
#define OUT stdout
#define num_global 1048576
#define num_local 1
...
int main(int argc, char *argv[]) {
	...
	for(int j = 0; j < num_global*num_local; j++) {
		...
		for(int i = 0; i < 4; i++) {
			output[i + 8*j] = 0xFFFFFFF0;
		}
		for(int i = 4; i < 8; i++) {
			output[i + 8*j] = 0x0000000F;
		}
	}
	...
	output_m = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int)*8*num_global*num_local, NULL, NULL);
	...
	while(1) {
		...
		err |= clEnqueueWriteBuffer(commands, output_m, CL_TRUE, 0, sizeof(int)*8*num_global*num_local, output, 0, NULL, NULL);
		if (err != CL_SUCCESS) {
